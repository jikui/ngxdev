...
static ngx_stream_upstream_resolved_t *
ngx_stream_alg_parse_alg_peer(u_char * addr_info, ssize_t size)
{
    ...
    peer = ngx_calloc(sizeof(ngx_stream_upstream_resolved_t), ngx_cycle->log);
    if (peer == NULL) {
        ngx_free(server_addr);
        return NULL;
    }

    peer->sockaddr = ngx_calloc(sizeof(struct sockaddr_in),ngx_cycle->log);
...
ngx_stream_alg_ftp_process(ngx_stream_session_t *s,u_char* buf,ssize_t size)
{
    ...
    if (ngx_strncmp(command +size -1 -2,CRLF,2) != 0 ) {
        ...
        if (entering_alg > 0) {
            ...
            if (entering_alg == 1) {
                ngx_snprintf(new_buf,120,"227 Entering Passive Mode (%ud,%ud,%ud,%ud,%ud,%ud).\r\n",addr1,addr2,addr3,addr4,port_num/256,port_num%256);
            }else {
                ngx_snprintf(new_buf,120,"PORT %ud,%ud,%ud,%ud,%ud,%ud\r\n",addr1,addr2,addr3,addr4,port_num/256,port_num%256);
            }
            ngx_log_debug2(NGX_LOG_DEBUG_STREAM,s->connection->log,0,
